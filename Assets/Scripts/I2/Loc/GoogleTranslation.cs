...
namespace I2.Loc
{
	public static class GoogleTranslation
	{
		...
		public static string ParseTranslationResult(string html, Dictionary<string, TranslationQuery> requests)
		{
			...
			{
				TranslationQuery translationQuery = FindQueryFromOrigText(text, requests);
				string text2 = array[num++];
				if (translationQuery.Tags != null)
				{
					for (int num3 = translationQuery.Tags.Length - 1; num3 >= 0; num3--)
					{
						text2 = text2.Replace(GetGoogleNoTranslateTag(num3), translationQuery.Tags[num3]);
					}
				}
				translationQuery.Results = text2.Split(separator, StringSplitOptions.None);
				if (TitleCase(text) == text)
				{
					for (int j = 0; j < translationQuery.Results.Length; j++)
					{
						translationQuery.Results[j] = TitleCase(translationQuery.Results[j]);
					}
				}
		...
		public static void AddQuery(string text, string LanguageCodeFrom, string LanguageCodeTo, Dictionary<string, TranslationQuery> dict)
		{
			...
			else
			{
				...
				if (Array.IndexOf(translationQuery2.TargetLanguagesCode, LanguageCodeTo) < 0)
				{
					translationQuery2.TargetLanguagesCode = translationQuery2.TargetLanguagesCode.Concat(new string[1]
					{
						LanguageCodeTo
					}).Distinct().ToArray();
				}
