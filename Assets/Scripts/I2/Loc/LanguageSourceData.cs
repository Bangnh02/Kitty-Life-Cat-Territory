...
namespace I2.Loc
{
	...
	public class LanguageSourceData
	{
		...
		public void Import_Google(bool ForceUpdate, bool justCheck)
		{
			if ((ForceUpdate || GoogleUpdateFrequency != eGoogleUpdateFrequency.Never) && I2Utils.IsPlaying())
			{
				...
				if (!ForceUpdate && googleUpdateFrequency != 0)
				{
					...
					try
					{
						if (DateTime.TryParse(setting_String, out DateTime result))
						{
		...
		{
			if (TryGetTranslation(term, out string Translation, overrideLanguage, overrideSpecialization, skipDisabled, allowCategoryMistmatch))
			{
				return Translation;
			}
			return string.Empty;
